...
import { useUpdateBand } from 'src/actions/dashboard/brand';

import { Form, Field } from 'src/components/hook-form';
import { CustomBreadcrumbs } from 'src/components/custom-breadcrumbs';

const BrandEditForm = ({ currentBrand }) => {
  ...
  const { updateBrand, updateBrandError, updateBrandLoading } = useUpdateBand();
  const handleSubmitBrandForm = async (data) => {
    try {
      ...
      await updateBrand({ brandId: currentBrand.id, payload: formData });
      if (updateBrandError) {
        toast.error(updateBrandError.message);
      } else {
        toast.success('Brand created successfully!');
        await mutate(endpoints.brand.list);
      }
    } catch (error) {
      ...
      <Form methods={methods} onReset={reset} onSubmit={handleSubmit(handleSubmitBrandForm)}>
        ...
        >
          <LoadingButton
            ...
            variant="contained"
            size="large"
            loading={updateBrandLoading || isSubmitting}
          >
            Save changes
          </LoadingButton>
        </Box>
